...
interface SalesPersonImportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImport: (data: any[]) => void;
}

export default function SalesPersonImportModal({ isOpen, onClose, onImport }: SalesPersonImportModalProps) {
  ...
  const handleDownloadTemplate = () => {
    ...
    XLSX.writeFile(wb, `业务员导入模板_${new Date().toLocaleDateString('zh-CN')}.xlsx`);
      ...
      reader.onload = async (e) => {
        try {
          ...
          const convertedData = jsonData.map((row: any) => ({
            name: row['姓名'] || '',
            position: row['职位'] || '销售顾问',
            department: row['部门'] || '',
            phone: row['手机号'] || '',
            email: row['邮箱'] || '',
            team: row['团队'] || '',
            join_date: row['入职日期'] || new Date().toISOString().split('T')[0],
            status: row['状态'] === '离职' ? 'disabled' : 'enabled',
            work_status: row['工作状态'] === '试用期' ? 'trial' : 
                        row['工作状态'] === '离职' ? 'resigned' : 'active',
            role: 'salesperson' // 固定为业务员角色
          }));
          ...
          convertedData.forEach((item: any, index: number) => {
            ...
            if (!item.department) {
              errors.push(`第${index + 2}行：部门不能为空`);
            }
          });
        ...
        <div className="p-6">
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6">
            ...
            <ul className="text-sm text-blue-800 dark:text-blue-400 space-y-1">
              ...
              <li>• 必填字段：姓名、手机号、部门</li>
              <li>• 手机号将用作登录账号，请确保唯一</li>
